...
- **自动重启**：监听工作区中的文件变化，比较文件哈希，仅在内容实际变更时触发重启命令。
...
| `restart-language-server.includes` | 要监听的 glob 模式数组。<br> **示例：** `["**/auto-imports.d.ts", "**/components.d.ts"]` | `["package.json", "package-lock.json", "pnpm-lock.yaml", ".yalc/**/package.json"]` |
